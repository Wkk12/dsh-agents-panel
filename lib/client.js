...
function RulesPanel({ theme }) {
  ...
  const [defaultRules, setDefaultRules] = (0, import_react.useState)([]);
  ...
  (0, import_react.useEffect)(() => {
    ...
    (async () => {
      try {
        ...
        setDefaultRules(lr.defaultRules || []);
  ...
  const chooseWs = (w) => {
    setWs(w);
    const saved = sels[w];
    setChecked(new Set(saved && saved.length ? saved : defaultRules));
    setMsg(null);
  };
  const toggle2 = (id) => {
    setChecked((c) => {
